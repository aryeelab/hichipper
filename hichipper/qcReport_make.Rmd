...
#opts_chunk$set(out.width='\\textwidth', dpi=200)
```
...
datatable(tab_summary, class = 'cell-border stripe')
```


The first summary table notes how many PETs are in each sample and what fraction were mapped with high-quality. The "% Long Range Interaction" value represents the number of intrachromosomal reads longer than the minimum length (default 5Kb) specified by the user and is a good approximation for the efficacy of the in situ ligation. Finally, the "% in Loops" is the fraction of total reads that are in loops, defined by high-quality, mapped intrachromosomal, anchor-mapped, between min and max lengths (default 5Kb and 2Mb). If the proportion of loops reads is low but the proportion of long range interactions is high, this suggests that the ChIP efficiency is relatively low. <br><br><br>

```{r, echo=FALSE, message=FALSE, warning=FALSE, out.width='\\textwidth'}
datatable(tab_summary2, class = 'cell-border stripe')
```
...
## Raw Read Stats
```{r, echo=FALSE, message=FALSE, warning=FALSE, out.width='\\textwidth'}
datatable(format(acast(readstats, metric~sample, sum), big.mark=",")[-8,, drop = FALSE], class = 'cell-border stripe')
```
<br>

## Proportional Read Stats
```{r, echo=FALSE, message=FALSE, warning=FALSE, out.width='\\textwidth'}
datatable(format(tab_percent[c(-8,-10),,drop = FALSE], digits=2, nsmall=2), class = 'cell-border stripe')
```
<br>
